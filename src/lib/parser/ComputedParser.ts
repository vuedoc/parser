...
export class ComputedParser extends AbstractExpressionParser {
      ...
      case Syntax.ObjectExpression:
        for (const item of node.properties) {
          switch (item.key.name) {
            ...
            case 'set':
              new EventParser(this.root, this.emitter, this.source, this.scope).parse(item);
              break;
          }
        }
        break;
    ...
    switch (node.type) {
      ...
      case Syntax.CallExpression:
        if (node.arguments.length) {
          this.parseComputedValue({ name, node: node.arguments[0], nodeComment, nodeTyping });
        }
        break;
  ...
  parseEntry(entry: Vuedoc.Entry.ComputedEntry, body, node, nodeComment = node, nodeTyping = null) {
    if (body) {
      if (node.returnType) {
  ...
  parseReturnType(node) {
    ...
    return returnNode
      ? this.parseType(returnNode.argument)
      : this.parseType(node);
  }
}
