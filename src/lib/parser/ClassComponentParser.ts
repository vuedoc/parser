...
export class ClassComponentParser extends ScriptParser<Babel.ClassDeclaration, any> {
  constructor(parent: ScriptParser<any, any>, root: Parser, ast: Vuedoc.Parser.AST.Result, source: Vuedoc.Parser.Script, options: Options) {
  ...
  parseClassProperty(node, defaultVisibility = Visibility.public) {
    ...
    if (!parsingResult && this.features.includes(Feature.data)) {
      ...
      new ClassComponentDataParser(this.root, this.emitter, this.source, this.scope).parse(node);
    }
  }
    ...
    switch (node.kind) {
      ...
      case 'get':
        if (this.features.includes(Feature.computed)) {
          new ClassComponentComputedParser(this.root, this.emitter, this.source, this.scope).parseNode(node);
        }
        break;
      ...
      case 'method':
        if (!this.parseClassMember(Type.unknown, node)) {
          ...
          if (ref.value === Feature.data) {
            new DataParser(this.root, this.emitter, this.source, this.scope).parse(node);
          } else {
  ...
  parseClassMember(type: Vuedoc.Parser.Type, node) {
    if (node.decorators) {
    ...
    switch (argument.type) {
      ...
      case Syntax.ObjectExpression: {
        ...
        if ('default' in ref.value) {
          prop.defaultValue = ref.rawObject?.default || JSON.stringify(ref.value.default);
  ...
  parsePropDecorator(node, [argument]: ArgumentNode[], type: Vuedoc.Parser.Type, name: string) {
    if (this.features.includes(Feature.props)) {
      const prop = {
        type,
        required: false,
      ...
      new PropParser(this.root, this.emitter, this.source, this.scope).parseEntry(entry, node, name);
    }
  }

  parsePropSyncDecorator(node, [propNameArg, propObjectArg]: ArgumentNode[], type: Vuedoc.Parser.Type, computedName: string) {
    ...
    if (this.features.includes(Feature.props)) {
      ...
      new PropParser(this.root, this.emitter, this.source, this.scope).parseEntry(propEntry, node, propName.value);
    }
  ...
  parseModelDecorator(node, [eventNameArg, propObjectArg]: ArgumentNode[], type: Vuedoc.Parser.Type, propName: string) {
    if (this.features.includes(Feature.props)) {
      ...
      new PropParser(this.root, this.emitter, this.source, this.scope).parseEntry(propEntry, node, propName);
    }
  }

  parseModelSyncDecorator(node, args: ArgumentNode[], type: Vuedoc.Parser.Type) {
    ...
    if (this.features.includes(Feature.computed)) {
      new ClassComponentComputedParser(this.root, this.emitter, this.source, this.scope).parseNode(node, type, [propName]);
    }

    if (this.features.includes(Feature.events)) {
      this.setScopeValue('value', node, new Value(type, 'value'));
      new EventParser(this.root, this.emitter, this.source, this.scope).parseEventNode(node, eventName, [
        {
  ...
  parseModelVModelDecorator(node, [propObjectArgNode]: ArgumentNode[], type: Vuedoc.Parser.Type) {
  ...
  parseRefDecorator(node, args: ArgumentNode[], type: Vuedoc.Parser.Type) {
  ...
  parseStateDecorator(node, args: ArgumentNode[], type: Vuedoc.Parser.Type) {
    if (this.features.includes(Feature.data)) {
      new ClassComponentDataParser(this.root, this.emitter, this.source, this.scope).parse(node, type);
    }
  }
  ...
  parseEmitDecorator(node, [eventNameArg]: ArgumentNode[], methodName: string) {
    ...
    if (this.features.includes(Feature.methods)) {
      new MethodParser(this.root).parseMethodProperty(node, node.body, node, {
        parseEvents: false,
      });
    }
    ...
    if (this.features.includes(Feature.events)) {
      ...
      new EventParser(this.root, this.emitter, this.source, this.scope)
    ...
    body.forEach((node) => {
      switch (node.expression.type) {
        case Syntax.CallExpression:
          new EventParser(this.root, this.emitter, this.source, this.scope).parse(node);
          break;
