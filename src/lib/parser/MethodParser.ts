...
function getBlockStatement(node): any {
  let block = null;
...
export class MethodParser extends AbstractExpressionParser {
  defaultVisibility: Vuedoc.Parser.Visibility;
  ...
  static parseEntrySyntax(entry: Vuedoc.Entry.PropFunction, node, { syntaxPrefix = '' } = {}) {
  ...
  parseMethodProperty(node, property, nodeComment = node, { parseEvents = true, hooks = LegacyHooks } = {}) {
    ...
    if (this.root.features.includes(Feature.methods)) {
      ...
      if (!hooks.includes(name as any)) { // ignore hooks
    ...
    if (parseEvents && this.features.includes(Feature.events)) {
      new EventParser(this.root, this.emitter, this.source, this.scope).parse(property);
    }
  }
