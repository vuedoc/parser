const { AbstractParser } = require('./parser/AbstractParser')
...
const JSDoc = {
  ...
  parseParams (keywords = [], params, generator) {
    ...
    AbstractParser.extractKeywords(keywords, TagAlias.param, true).forEach(({ description }) => {
  ...
  parseReturn (keywords = [], returns) {
    AbstractParser.extractKeywords(keywords, TagAlias.returns, true).forEach(({ description }) => {
  ...
  parseReturnKeyword (text, type = Type.any) {
    const output = { type, description: undefined }
