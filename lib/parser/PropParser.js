        ...
        if (node.callee.object.name === PROP_TYPES_OBJECT_NAME) {
          ...
          switch (node.callee.property.name) {
            ...
            case PROP_TYPES_ARRAY_OF: {
              const type = this.parsePropTypeMember(argument)

              return type
                ? `${type}[]`
                : this.getSourceString(argument)
            }
            ...
            case PROP_TYPES_OBJECT_OF:
              return this.parsePropTypeMember(argument)

            case PROP_TYPES_SHAPE:
              return this.getSourceString(argument)
                .replace(/\s+/g, ' ')
                .replace(new RegExp(`${PROP_TYPES_OBJECT_NAME}\\.`, 'g'), '')
          }
        }
        break

      default:
        // type = this.getSourceString(node.value)
    }
