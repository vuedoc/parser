...
const { Syntax, DEFAULT_TYPE } = require('../Enum')
...
class PropParser extends AbstractExpressionParser {
  static parseEntryCommentType (entry) {
    const items = AbstractExpressionParser.extractKeywords(entry.keywords, 'type')
        ...
        if (node.callee.object.name === PROP_TYPES_OBJECT_NAME) {
          ...
          switch (node.callee.property.name) {
            ...
            case PROP_TYPES_ARRAY_OF: {
              const type = this.parsePropTypeMember(argument) || DEFAULT_TYPE
            ...
            case PROP_TYPES_OBJECT_OF:
              return this.parsePropTypeMember(argument) || DEFAULT_TYPE
  ...
  getPropType (node) {
    ...
    const value = ref.value.default ? this.parseValue(ref.value.default) : undefined
