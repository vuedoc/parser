...
const { Syntax, Type } = require('../Enum');
...
class ComputedParser extends AbstractExpressionParser {
  ...
  parseReturnType (node, parent) {
    if (parent.returnType) {
      return this.getTSType(parent.returnType, Type.unknow);
    }
    ...
    if (returnNode) {
      switch (returnNode.argument.type) {
        case Syntax.BinaryExpression:
          return AbstractExpressionParser.parseType(returnNode.argument.left);

        default:
          return AbstractExpressionParser.parseType(returnNode.argument);
      }
    }

    return AbstractExpressionParser.parseType(node);
  }
}
