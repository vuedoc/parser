...
const COMPOSITION_API_METHODS = ['ref', 'computed'];
...
export class ScriptParser extends AbstractParser {
  ...
  parseImportDeclaration(item) {
    if (this.source.attrs.setup) {
      if (item.importKind === 'value' && item.source.value === 'vue') {
        item.specifiers
          .filter(({ imported }) => COMPOSITION_API_METHODS.includes(imported.name))
          .forEach(() => {});
      ...
      switch (node.callee.name) {
        case 'withDefaults':
          if (node.arguments[0]?.type === Syntax.CallExpression) {
            this.parseCallExpression(node.arguments[0]);
          }
          break;
  ...
  parseVariableDeclaration(node) {
    ...
    if (this.source.attrs.setup) {
      // console.log('>>> parseVariableDeclaration:>', node);
      ...
      node.declarations.forEach((declaration) => {
        ...
        switch (value.$kind) {
          ...
          case 'computed':
            new ComputedParser(this).parseComputedValue({ name, node: declaration.init.arguments[0], nodeTyping, nodeComment });
            break;
