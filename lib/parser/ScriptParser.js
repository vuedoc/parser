...
const { Syntax, Properties, Feature } = require('../Enum')
...
class ScriptParser extends AbstractParser {
  ...
  findComment ({ trailingComments = [], leadingComments = trailingComments }) {
  ...
  parseComment (description, keywords) {
    if (this.root.features.includes(Feature.description)) {
      ...
      AbstractParser.parseEntryDescription(entry)
    ...
    if (this.root.features.includes(Feature.name)) {
      if (keywords.length) {
        const nameKeywords = KeywordsUtils.extract(keywords, 'name', true)
