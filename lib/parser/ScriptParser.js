...
import { Syntax, Properties, Feature, Tag, PropTypesTag, Type, Visibility, CompositionAPIValues, CompositionAPI, CompositionHooks, RouterKeys, CompositionProperties } from '../Enum.js';
...
export class ScriptParser extends AbstractParser {
  ...
  parseObjectExpression(node) {
    ...
    if (this.publicProperties?.length) {
      this.source.attrs.setup = true;
    } else if (!this.source.attrs.setup) {
      this.source.attrs.setup = node.properties.some(({ key: { name } }) => SPECIFIC_COMPOSITION_PROPERTIES.includes(name));
    }
      ...
      case Properties.watch:
        if (this.features.includes(Feature.events)) {
          property.value.properties
            ...
            .forEach((watcher) => new EventParser(this).parse(watcher));
        }
        break;
