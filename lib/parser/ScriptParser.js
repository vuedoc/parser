...
class ScriptParser extends AbstractParser {
  ...
  constructor (root) {
    ...
    this.source = ScriptParser.escapeImportKeyword(root.options.source.script)
    this.ast = acorn.parse(this.source, options)
  }

  static escapeImportKeyword (code) {
    return code.replace(/import\s*\(/g, 'xmport(')
  }
  ...
  findToken (node) {
    const index = this.findTokenIndex(node)
