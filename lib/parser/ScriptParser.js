...
export class ScriptParser extends AbstractParser {
  ...
  parseObjectExpression(node) {
    const exposeProperty = node.properties.find((property) => property.key.name === 'expose');

    if (exposeProperty && exposeProperty.value) {
      const properties = this.getValue(exposeProperty.value);
