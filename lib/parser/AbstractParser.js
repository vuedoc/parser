...
const NullValue = new Value('any', 'null')
...
class AbstractParser {
  ...
  getSourceValue (node, type = node.type) {
    return new Value(type, this.getSource(node))
  }

  getTemplateLiteralValue (node) {
    return node.quasis
      ? this.getSourceValue(node)
      : this.getSourceValue(node.quasi)
  }
  ...
  getValue (node) {
    ...
    switch (node.type) {
      ...
      case Syntax.UnaryExpression:
        return this.getSourceValue(node, typeof node.argument.value)

      case Syntax.CallExpression:
        return this.getSourceValue(node, node.callee.name)

      case Syntax.MemberExpression:
        return this.getSourceValue(node, node.object.name === 'Number' ? node.object.name : 'Object')
