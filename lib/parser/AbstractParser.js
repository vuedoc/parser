...
import { Syntax, Type, TypedocTag, JSDocTag, TypeList } from '../Enum.js';
...
export class AbstractParser {
    ...
    for (const item of node.properties) {
      switch (item.type) {
        case Syntax.ObjectProperty: {
          ...
          if (item.value.type === Syntax.Identifier) {
  ...
  getMemberExpression(node) {
    ...
    if (type === Type.object && node.object.name in this.scope) {
      const key = this.getSourceString(node.property);
      const object = this.scope[node.object.name];
      const value = get(object.value, key);
      const raw = get(object.raw, key);
  ...
  getCallExpression(node) {
    ...
    switch (node.callee.name) {
      ...
      default:
        if (COMPOSITION_API.includes(node.callee.name)) {
          value = node.arguments.length ? this.getValue(node.arguments[0]) : UndefinedValue;
        }
    }
  ...
  getArrayExpression(node) {
    ...
    return new Value(Type.array, value, this.getInlineSourceString(node));
  }
