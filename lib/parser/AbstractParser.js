...
const { Syntax, Type, TypedocTag, JSDocTag, TagAlias } = require('../Enum')
...
class AbstractParser {
  ...
  static extractKeywords (keywords, keywordNames, withNotEmptyDesc = false) {
  ...
  static parseEntryDescription (entry) {
    const keywords = AbstractParser.extractKeywords(entry.keywords, TagAlias.description, true)
    ...
    if (keywords.length) {
      ...
      entry.description += keywords.map(({ description }) => description).join('\n\n')
