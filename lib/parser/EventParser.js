...
const { DEFAULT_TYPE, Syntax } = require('../Enum')
...
class EventParser extends AbstractExpressionParser {
        ...
        switch (node.callee.property.name) {
          case '$emit':
            if (node.arguments.length) {
              ...
              this.parseEventNode(node, name, eventArgs, parent)
            }
            break
  ...
  parseEventNode (node, name, args, parent = node) {
    ...
    this.parseEntryNode(node, name, parsedArgs, parent)
  }

  parseEntryNode (node, name, args, parent = node) {
