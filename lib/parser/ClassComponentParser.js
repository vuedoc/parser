...
const { Syntax, Type, Features, Visibilities } = require('../Enum')
...
class ClassComponentParser extends ScriptParser {
    ...
    node.body.body.forEach((property) => {
      ...
      switch (property.type) {
        ...
        case Syntax.ClassPrivateMethod:
          this.parseClassMethod(property, Visibilities.private)
          break
      }
    })
  }
  ...
  parseClassProperty (node) {
    ...
    if (!parsingResult && this.features.includes(Features.data)) {
      ...
      const value = this.parseValue(ref)
      const dataEntry = new DataEntry(node.key.name, { type, value })
  ...
  parseClassMethod (node, defaultVisibility = this.emitter.options.defaultMethodVisibility) {
    switch (node.kind) {
      ...
      case 'method':
        if (!this.parseClassMember(Type.any, node)) {
