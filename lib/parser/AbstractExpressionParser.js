...
const { Syntax, Visibility, Type } = require('../Enum');
  ...
  getTSType (node, defaultType = Type.any) {
    switch (node.type) {
      case Syntax.TSAsExpression:
        return this.getTSType(node.typeAnnotation);

      case Syntax.TSTypeReference:
        return node.typeName.name === PROP_OPTIONS
          ? this.getTSType(node.typeParameters)
          : this.getInlineSourceString(node);

      case Syntax.TSTypeParameterInstantiation:
        return node.params.length
          ? this.getInlineSourceString(node.params[0])
          : defaultType;

      case Syntax.TSFunctionType:
        return this.getInlineSourceString(node)
          .replace(/\(\s+/g, '(')
          .replace(/\s+\)/g, ')');

      default:
        return node.typeAnnotation
          ? this.getInlineSourceString(node.typeAnnotation.typeAnnotation)
          : defaultType;
    }
  }
