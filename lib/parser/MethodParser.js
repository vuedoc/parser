...
const { Syntax, Feature, Properties, Type, Visibility } = require('../Enum')
...
function parseMethodName (entry) {
  const [ keyword ] = KeywordsUtils.extract(entry.keywords, 'method', true)
...
function parseEntrySyntax (entry, node) {
  const syntaxKeywords = KeywordsUtils.extract(entry.keywords, 'syntax', true)
...
class MethodParser extends AbstractExpressionParser {
  ...
  parseEntryReturns ({ keywords, returns }, node) {
    ...
    JSDoc.parseReturn(keywords, returns)
  }
}
