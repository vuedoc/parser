...
class MarkupTemplateParser extends AbstractParser {
  ...
  getSiblingNode (node) {
    const reversedTokens = this.nodes.reverse()
  ...
  parseSlot (node) {
    ...
    const slots = AbstractSlotParser.extractSlotKeywords(comment.keywords)

    if (slots.length) {
      slots.forEach((slot) => this.emit(slot))
    } else {
      AbstractSlotParser.parseTemplateSlots(entry, node.attrsList, comment)
      this.emit(entry)
    }
  }
