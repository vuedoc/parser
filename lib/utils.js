...
const JSDOC_PARAM_NAME = '[a-z0-9$\\.\\[\\]_]+'
const JSDOC_TYPE = '[a-z\\[\\]|\\. \\*]*'
const JSDOC_PARAM_RE = new RegExp(`^\\s*(\\{\\(?(${JSDOC_TYPE})\\)?\\}\\s+)?(${JSDOC_PARAM_NAME}|\\[(${JSDOC_PARAM_NAME})(=(.*))?\\])\\s+-?\\s*(.*)`, 'i')
const JSDOC_RETURN_RE = new RegExp(`^\\s*(\\{\\(?(${JSDOC_TYPE})\\)?\\}\\s+)?-?(.*)`, 'i')
const JSDOC_DEFAULT_TYPE = 'Any'

const parseJsdocType = (type, param) => {
  if (type.indexOf('|') > -1) {
...
const parseParamKeyword = (text) => {
  ...
  if (matches) {
    if (matches[2]) {
      parseJsdocType(matches[2], param)
    }
...
const parseReturnKeyword = (text) => {
  ...
  if (matches[2]) {
    parseJsdocType(matches[2], output)
  }
...
module.exports.parseJsdocType = parseJsdocType
