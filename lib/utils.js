...
const DEFAULT_FEATURES = [ 'description', 'keywords' ]

const MODEL_KEYWORD = 'model'

function getVisibility (keywords, defaultVisibility) {
...
function parseComment (text, defaultVisibility = DEFAULT_VISIBILITY, features = DEFAULT_FEATURES) {
  const result = {
    keywords: [],
...
function getComment (property, defaultVisibility = DEFAULT_VISIBILITY, features) {
...
function getIdentifierValue (tokens, identifierName, rangeLimit) {
...
function getIdentifierValueFromStart (tokens, identifierName) {
...
function getDependencies (ast, source) {
  const dependencies = []
...
function escapeImportKeyword (code) {
