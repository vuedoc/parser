...
const getVisibility = (keywords, defaultVisibility) => {
  const item = keywords.find((item) => RE_VISIBILITY.test(item.keyword))
...
const extractKeywords = (description) => {
  ...
  while (index < description.length && index !== -1) {
    const matches = RE_KEYWORDS.exec(description)
...
const extractKeywordsAsync = (description) => {
...
const parseComment = (text, defaultVisibility) => {
  ...
  const visibility = getVisibility(keywords, defaultVisibility)
  const describeModel = hasModelDecoration(text)
  const description = text.replace(RE_VISIBILITY, '')

  const parsedComment = {
    keywords, visibility, describeModel, description
  }
...
module.exports.extractKeywordsAsync = extractKeywordsAsync
