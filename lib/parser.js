...
class Parser extends EventEmitter {
  constructor (options) {
    ...
    this.ast = options.source.hasOwnProperty('script') && options.source.script
      ? Parser.parsingScript(options.source.script, options)
      : null
  ...
  static parsingScript (source, options) {
    let ast = null

    try {
      ast = espree.parse(source, options)
    } catch (e) {
      const script = utils.escapeImportKeyword(source)

      ast = espree.parse(script, options)
    }
    ...
    property.value.properties.forEach((p) => {
      ...
      if (p.type === 'ExperimentalSpreadProperty') {
        if (p.argument.name in this.identifiers) {
          ...
          this.parseObjectExpression(spreadProperty)
      ...
      Parser.parseKeywords(entry.keywords, entry)
  ...
  parseAstBody (body) {
    const entry = utils.getComment(
      body, this.defaultMethodVisibility, this.features
    )
    ...
    if (!EXPORT_DECLARATION.includes(body.type)) {
      if (body.type === 'VariableDeclaration') {
        const declaration = body.declarations[0]

        this.identifiers[declaration.id.name] = declaration.init
      }
      ...
      switch (body.declaration.type) {
        case 'ObjectExpression':
          body.declaration.properties.forEach((property) => {
            return this.extractProperties(property)
          })
          break

        case 'Identifier':
          if (this.identifiers.hasOwnProperty(body.declaration.name)) {
            const { properties } = this.identifiers[body.declaration.name]
    ...
    } else if (body.expression !== null && body.expression.right) {
      if (body.expression.right.properties) {
        body.expression.right.properties.forEach((property) => {
          return this.extractProperties(property)
        })
      }
    }
  ...
  walk () {
    process.nextTick(() => {
      if (this.features.length) {
        ...
        if (this.ast) {
          this.ast.body.forEach((body) => this.parseAstBody(body))
        } else if (this.features.includes('name')) {
          this.parseComponentName()
        }
      }
    ...
    if (entry instanceof utils.NodeFunction) {
      ...
      for (let i = 0; i < tokens.length; i++) {
        ...
        if (token.type === 'Identifier' && token.value === '$emit') {
          ...
          if (event.description) {
            ...
            if (keyword) {
              if (!keyword.description) {
                const message = 'Missing keyword value for @event'
          ...
          Parser.parseKeywords(event.keywords, event)
        ...
        if (name === 'slot') {
          ...
          Object.keys(attrs)
            ...
            .forEach((event) => {
              ...
              Parser.parseKeywords(comment.keywords, event)
              this.emit('event', event)
